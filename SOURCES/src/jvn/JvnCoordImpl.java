...
public class JvnCoordImpl extends UnicastRemoteObject implements JvnRemoteCoord {
	 ...
	 *             ,JvnException
	 **/
	public synchronized void jvnRegisterObject(String jon, JvnObject jo,
			JvnRemoteServer js) throws java.rmi.RemoteException,
			jvn.JvnException {
		...
		if (naming.containsKey(jon)) {
			...
			for (Map.Entry<JvnCodeOS, JvnStatus> entry : locktable.entrySet()) {
				if (entry.getKey().getJoi() == jvnObject.jvnGetObjectId()
					...
					Serializable objet = ts.jvnInvalidateWriter(jvnObject
							.jvnGetObjectId());
	...
	public synchronized Serializable jvnLockRead(int joi, JvnRemoteServer js)
		...
		for (Map.Entry<JvnCodeOS, JvnStatus> entry : locktable.entrySet()) {
			...
			if (entry.getKey().getJoi() == joi
				...
				else if (entry.getValue().getState().equals(LockState.W)
						|| entry.getValue().getState().equals(LockState.RWC)) {
			...
			} else if (entry.getKey().getJoi() == joi
					&& entry.getKey().getJs().equals(js)) {
	...
	public synchronized Serializable jvnLockWrite(int joi, JvnRemoteServer js)
		...
		for (Map.Entry<JvnCodeOS, JvnStatus> entry : locktable.entrySet()) {
			...
			if (entry.getKey().getJoi() == joi) {
				...
				System.out
						.println("Objet non referencie dans la base retour NULL");
			}
		}
	...
	public HashMap<String, JvnJonObj> getNaming() {
		return this.naming;
	}

	public HashMap<JvnCodeOS, JvnStatus> getLocktableser() {
	...
	public void updateSlave() {
		...
		try {
			...
			if (list.length == 2) { // tester s'il y a un coordinateur et un
									// slave dans le registry
				jvnCoordImpl = (JvnRemoteCoord) Naming
						.lookup("rmi://localhost:1099/slave");
				try {
					jvnCoordImpl.jvnUpdateCache(naming, locktable); // mettre
																	// ï¿½ jour
																	// les
																	// donnï¿½es
																	// du slave
	...
	public static void main(String argv[]) {
		...
		try {
			...
			// cordinateur 2
			try { // si le coordinateur est deja lancé
				JvnRemoteCoord jvnCoord = (JvnRemoteCoord) Naming
				...
				if (list.length == 1) {
					...
					jvnCoordImpl.updateSlave();// mettre a jour les donnï¿½es du
												// slave
	...
	// Mettre ï¿½ jour la cache du slave
	public void jvnUpdateCache(HashMap<String, JvnJonObj> naming,
			HashMap<JvnCodeOS, JvnStatus> locktable) throws RemoteException,
			JvnException {
		this.locktable = locktable;
		this.naming = naming;
